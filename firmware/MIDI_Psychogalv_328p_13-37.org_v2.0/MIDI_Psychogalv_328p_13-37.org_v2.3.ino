...
uint16_t channel = 1;  //setting channel to 11 or 12 often helps simply computer midi routing setups // SETTING
float threshold = 2.3;   //2.3;  //change threshold multiplier // SETTING
...
const byte buttonPin = 13; //tact button input
int menus = 4; //number of main menus
int mode = 0; //0 = Threshold, 1 = Scale, 2 = Brightness, 3 = Channel
int currMenu = 0;
...
unsigned long batteryCheck = 5000; //battery check delay timer
...
void midiSerial(int type, int channel, int data1, int data2) {
  ...
  data1 &= 0x7F;  //number
  data2 &= 0x7F;  //velocity
  // commented out to make channel adjustment work - needs more testing
...
int scaleNote_slow(int note, int scale[], int root) {
  //input note mod 12 for scaling, note/12 octave
  //search array for nearest note, return scaled*octave
  int scaled = note % 12;
  int octave = note / 12;
